...
namespace PLAYER
{
    ...
    public class PlayerGrappler
    {
        ...
        public void Initialize(PlayerController player, LineRenderer lineRenderer, DistanceJoint2D distanceJoint2D, LayerMask grapplerLayer, Rigidbody2D rigidBody2D)
        {
            this.player = player;
        ...
        public void Grapple(InputAction.CallbackContext context)
        {
            if (context.started)
            {
                ...
                if (hit.collider != null)
                {
                    ...
                    if (distanceToGrapplePoint <= grappleMaxPoint)
                    {
                        ...
                        // Adiciona rotação ao jogador
                        rigidBody2D.angularVelocity = 360f; // Velocidade de rotação em graus por segundo
                    }
                    else
                    {
            ...
            else if (context.canceled)
            {
                ...
                // Para a rotação quando o grappler é cancelado
